"""
A Dispatcher maintains a list of currently
...
This is the base dispatcher -- it does no actual dispatching,
but provides a framework on which to base other dispatchers.
"""
...
class BaseDispatcher(KeepRefs, LoggerMixin):
    ...
    def handle(self, msg):
        ...
        patient = get_patient_by_msg(msg)
    ...
    def get_patient_by_msg(msg):
        """
        Gets the patient using the identity information in the msg.connection object.

        A NameError exception is raised if the patient doesn't exist or the backend not in our recognized set.
        """
        ...
        if backend == u'sms' or backend == u'clickatell':
            ...
            raise NameError("Unrecognizable backend")
