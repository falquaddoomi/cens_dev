"""
A Keyword-based Dispatcher; it determines which
of the running tasks it manages should receive a
message by examining a space-delimited prefix
keyword that each task registers when it loads
(or whenever the task decides to change it).

The keyword is automatically appended to
outgoing messages unless there is only one
running task, in which case no keyword is
necessary (but can still be included and will
be ignored).
"""
...
class KeywordTaskDispatcher(BaseDispatcher):
